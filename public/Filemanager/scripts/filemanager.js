...
var capabilities = config.options.capabilities || new Array('playlist','select', 'download', 'rename', 'move', 'delete', 'replace');
...
var bindToolbar = function(data) {
	...
	if (!has_capability(data, 'playlist')) {
		$('#fileinfo').find('button#playlist').hide();
	} else {
		$('#fileinfo').find('button#playlist').click(function(){
...
var renameItem = function(data) {
	...
	var msg = lg.new_filename+' : <input id="rname" name="rname" type="text" value="' + fileName + '"disabled />';
...
var add_to_playlist=function(data) {
...
function getContextMenuOptions(elem) {
	...
	if (!($('#' + optionsID).length)) {
		...
		if (!elem.hasClass('cap_playlist')) $('.playlist', newOptions).remove();
...
var getFileInfo = function(file) {
	...
	if($.inArray('rename', capabilities)  != -1 && config.options.browseOnly != true) template += '<button id="rename" name="rename" type="button" value="Rename">Add_to_playlist</button>';
		if($.inArray('playlist', capabilities)  != -1 && config.options.browseOnly != true) template += '<button id="playlist" name="playlist" type="button" value="playlist">' + lg.playlist + '</button>';
...
$(function(){
	...
	if(config.options.autoload == true) {
		...
		$('#itemOptions a[href$="#playlist"]').append(lg.playlist);
